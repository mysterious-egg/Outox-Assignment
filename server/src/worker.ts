...
const worker = new Worker(
  ...
  async (job) => {
    ...
    const rateLimitResult = await reserveEmailSlot();

    if (!rateLimitResult.allowed) {
      console.log(
        `Hourly rate limit reached. Delaying job ${job.id} until ${rateLimitResult.nextWindowAt.toISOString()}`
      );

      await job.moveToDelayed(
        rateLimitResult.nextWindowAt.getTime(),
        job.token
      );

      throw new DelayedError(
        "Job moved to the next hourly rate-limit window"
      );
    }

    console.log(
      `Rate-limit slot reserved for email ${email.id}.`
    );
