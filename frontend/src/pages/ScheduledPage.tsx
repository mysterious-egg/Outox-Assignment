...
function ScheduledPage() {
  ...
  async function fetchScheduledEmails() {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.message || "Failed to fetch scheduled emails",
        );
      }
                ...
                {filteredEmails.map((email) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="rounded-full bg-[#eef8f3] px-3 py-1 text-xs font-semibold text-[#1f8f5f]">
                        Scheduled
                      </span>
                    </td>
                  </tr>
